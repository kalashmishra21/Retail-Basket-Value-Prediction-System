...
class ModelLoader:
    ...
    def predict(self, features_dict):
        """
        Make prediction using loaded models with scientific confidence calculation.
        ...
        # Use scientific confidence calculator if available
        try:
            from ml.confidence_calculator import get_confidence_calculator
            
            calc = get_confidence_calculator()
            confidence_result = calc.calculate_confidence(
                features_dict, pred_main, pred_lower, pred_upper
            )
            outlier_result = calc.is_outlier(features_dict, confidence_result)
            
            confidence = confidence_result['confidence']
            is_outlier = outlier_result['is_outlier']
            outlier_score = outlier_result['outlier_score']
            
            # Log confidence factors for debugging
            print(f"  Confidence factors: {confidence_result['confidence_factors']}")
            if outlier_result['outlier_reasons']:
                print(f"  Outlier reasons: {outlier_result['outlier_reasons']}")
            
        except Exception as e:
            # Fallback to simple CV-based calculation
            print(f"  Warning: Using fallback confidence calculation: {e}")
            ...
            is_outlier = cv > 0.50
            outlier_score = min(cv * 100, 100.0) if is_outlier else 0.0
